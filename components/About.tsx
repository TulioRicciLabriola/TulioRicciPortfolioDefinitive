      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
        ...
        <div className="lg:pt-28">
          <ul className="space-y-12">
            {items.map((item, idx) => (
              <li key={idx} className="flex items-start gap-6 group cursor-default" onMouseEnter={() => setActiveIndex(idx)}>
                <div className={`w-2.5 h-2.5 mt-2 rounded-full flex-shrink-0 transition-all duration-500 ${activeIndex === idx ? 'bg-white shadow-[0_0_15px_rgba(255,255,255,1)] scale-125' : 'bg-white/20 scale-100 group-hover:bg-white/40'}`}></div>
                <div>
                  <h4 className="font-bold uppercase tracking-[0.2em] text-sm mb-2 text-white">{item.title}</h4>
                  <p className="text-[14px] leading-relaxed font-light text-gray-300">{item.desc}</p>
                </div>
              </li>
            ))}
          </ul>
          <div className="mt-20 p-10 bg-white/[0.03] border border-white/10 relative overflow-hidden group">
            ...
            <p className="italic text-gray-300 text-base leading-relaxed font-light relative z-10">{t.about.quote}</p>
          </div>
        </div>
      </div>
    </section>
