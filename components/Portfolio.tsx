          ...
          return (
            <article key={uniqueId} className="group flex flex-col animate-[fadeIn_0.8s_ease-out_forwards]" style={{ animationDelay: `${(index % itemsPerPage) * 0.15}s` }}>
                  ...
                  <div className="w-full h-full relative overflow-hidden group">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                       <div className="w-20 h-20 rounded-full border border-white/20 flex items-center justify-center bg-black/40 backdrop-blur-md group-hover:bg-white group-hover:border-white transition-all duration-500 scale-90 group-hover:scale-100 shadow-2xl">
                          <svg className="w-8 h-8 text-white group-hover:text-black ml-1.5 transition-colors" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                       </div>
                    </div>
                  </div>
                )}
              ...
              <div className="flex flex-col">
                <span className="text-[11px] text-white/20 font-mono mb-2 tracking-[0.4em] uppercase">Project #0{index + 1}</span>
                <h3 className="text-2xl md:text-3xl font-bold tracking-tight text-white/90 group-hover:text-white transition-colors uppercase leading-tight mb-3">{item.title}</h3>
                <p className="text-gray-400 text-[14px] leading-relaxed line-clamp-2 font-light border-l border-white/10 pl-5">{item.description}</p>
              </div>
            </article>
          );
        })}
