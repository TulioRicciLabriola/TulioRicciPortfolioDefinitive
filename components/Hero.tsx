...
const Hero: React.FC<HeroProps> = ({ onNavClick }) => {
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-center px-6 md:px-12 lg:px-24 pt-20 overflow-hidden bg-[#0E0E11]">
      ...
      <div className="relative z-30 max-w-4xl">
        <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold leading-[1.05] mb-8 tracking-tighter text-[#F1F1F1] drop-shadow-[0_10px_30px_rgba(0,0,0,0.8)]">
          {t.hero.title1} <br />
          <span className="text-white/40">{t.hero.titleHighlight}</span> <br />
          {t.hero.title2}
        </h1>
        
        <p className="text-lg md:text-xl text-[#F1F1F1]/90 font-light mb-12 max-w-xl leading-relaxed drop-shadow-xl">
          {t.hero.subtitle}
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-10">
          <button 
            ...
            className="px-14 py-5 bg-[#F1F1F1] text-black text-center font-bold uppercase tracking-[0.2em] text-[10px] hover:bg-white transition-all active:scale-95 shadow-[0_20px_50px_rgba(0,0,0,0.6)]"
          >
            {t.hero.ctaWork}
          </button>
          <button 
            ...
            className="text-[#F1F1F1]/70 hover:text-white transition-all font-bold uppercase tracking-[0.3em] text-[10px] border-b border-transparent hover:border-white/20 pb-1"
          >
            {t.hero.ctaContact}
          </button>
        </div>
      ...
      <button 
        type="button"
        onClick={(e) => onNavClick(e, 'portfolio')}
        className="absolute bottom-12 left-6 md:left-12 lg:left-24 flex items-center gap-6 group cursor-pointer z-30 opacity-40 hover:opacity-100 transition-opacity"
      >
